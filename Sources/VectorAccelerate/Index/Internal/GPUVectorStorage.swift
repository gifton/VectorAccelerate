...
final class GPUVectorStorage: @unchecked Sendable {
    ...
    /// - Throws: `IndexError` if buffer allocation fails
    ...
    private func allocateBuffer() throws {
        ...
        guard let newBuffer = device.makeBuffer(length: bufferSize, options: .storageModeShared) else {
            throw IndexError.gpuResourceCreationFailed(
                index: "GPUVectorStorage",
                reason: "Failed to allocate \(bufferSize) bytes for vector buffer"
            )
        }
        newBuffer.label = "VectorAccelerate.VectorStorage"
        buffer = newBuffer
    }
    ...
    ///
    /// - Throws: `IndexError` if reallocation fails
    func grow() throws {
    ...
    /// - Throws: `IndexError` if growth fails
    ...
    /// - Throws: `IndexError` if buffer not initialized or index out of bounds
    ...
    /// - Throws: `IndexError` if buffer not initialized or validation fails
    func writeVectors(_ vectors: [[Float]], startingAt startSlot: Int) throws {
        guard let buffer = buffer else {
            throw IndexError.gpuNotInitialized(operation: "writeVectors")
        }
    ...
    func readVector(at slotIndex: Int) throws -> [Float] {
        guard let buffer = buffer else {
            throw IndexError.gpuNotInitialized(operation: "readVector")
        }
    ...
    /// - Throws: `IndexError` if compaction fails
    @discardableResult
    func compact(keepMask: [Bool]) throws -> [Int: Int] {
        ...
        newBuffer.label = "VectorAccelerate.VectorStorage.Compacted"
    ...
    /// - Throws: `IndexError` if read fails
