...
public struct KMeansConfiguration: Sendable {
    ...
    public func validate() throws {
        guard numClusters >= 1 else {
            throw IndexAccelerationError.invalidInput(message: "numClusters must be at least 1, got \(numClusters)")
        }
        guard numClusters <= 65536 else {
            throw IndexAccelerationError.invalidInput(message: "numClusters must be at most 65536, got \(numClusters)")
        }
        guard dimension >= 1 && dimension <= 4096 else {
            throw IndexAccelerationError.invalidInput(message: "dimension must be 1-4096, got \(dimension)")
        }
        guard maxIterations >= 1 else {
            throw IndexAccelerationError.invalidInput(message: "maxIterations must be at least 1, got \(maxIterations)")
        }
        guard convergenceThreshold > 0 else {
            throw IndexAccelerationError.invalidInput(message: "convergenceThreshold must be positive, got \(convergenceThreshold)")
        }
    }
