...
public final class IVFCoarseQuantizerKernel: @unchecked Sendable, Metal4Kernel {
    ...
    public func findNearestCentroids(
        ...
        guard numQueries > 0 else {
            throw IndexAccelerationError.invalidInput(message: "numQueries must be positive")
        }
        guard numCentroids > 0 else {
            throw IndexAccelerationError.invalidInput(message: "numCentroids must be positive")
        }
        guard nprobe > 0 && nprobe <= numCentroids else {
            throw IndexAccelerationError.invalidInput(message: "nprobe must be between 1 and numCentroids")
        }
    ...
    public func findNearestCentroids(
        ...
        guard !centroids.isEmpty else {
            throw IndexAccelerationError.invalidInput(message: "Centroids cannot be empty")
        }
