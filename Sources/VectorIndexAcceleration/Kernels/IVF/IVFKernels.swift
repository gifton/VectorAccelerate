...
public struct IVFSearchConfiguration: Sendable {
    ...
    public func validate() throws {
        guard numCentroids >= 1 else {
            throw IndexAccelerationError.invalidInput(message: "numCentroids must be at least 1, got \(numCentroids)")
        }
        guard nprobe >= 1 else {
            throw IndexAccelerationError.invalidInput(message: "nprobe must be at least 1, got \(nprobe)")
        }
        guard nprobe <= numCentroids else {
            throw IndexAccelerationError.invalidInput(message: "nprobe (\(nprobe)) cannot exceed numCentroids (\(numCentroids))")
        }
