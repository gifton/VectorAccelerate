...
final class NeuralQuantizationKernelTests: XCTestCase {
    ...
    // NOTE (VA3-021): the tiled V1/V2 encode test family (10 tests + 5 helpers)
    // was deleted with the APIs it exercised — every one skipped on
    // `isTiled*Available` guards that could never be true, because the kernels
    // never existed in any .metal file. They accounted for ~half of the suite's
    // standing skip count while reading as coverage.
