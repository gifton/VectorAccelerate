...
final class IVFValidationTests: XCTestCase {
    ...
    func testRecallIncreasesMonotonicallyWithNprobe() async throws {
        ...
        for query in queries {
            ...
            groundTruth.append(Set(results.map { $0.id.stableID }))
        }
        ...
        for nprobe in nprobeValues {
            ...
            for (i, query) in queries.enumerated() {
                ...
                let ivfIndices = Set(results.map { $0.id.stableID })
    ...
    func testFullNprobeGivesNearPerfectRecall() async throws {
        ...
        for query in queries {
            ...
            let gtIndices = Set(gtResults.map { $0.id.stableID })

            let ivfResults = try await ivfIndex.search(query: query, k: k)
            let ivfIndices = Set(ivfResults.map { $0.id.stableID })
    ...
    func testDistancesMatchFlatIndex() async throws {
        ...
        for query in queries {
            ...
            let flatDistances = Dictionary(uniqueKeysWithValues: flatResults.map { ($0.id.stableID, $0.distance) })
            let ivfDistances = Dictionary(uniqueKeysWithValues: ivfResults.map { ($0.id.stableID, $0.distance) })
    ...
    func testIndexMappingIsCorrect() async throws {
        ...
        for (i, idx) in landmarkIndices.enumerated() {
            ...
            let foundIndices = results.map { Int($0.id.stableID) }
    ...
    func testDuplicateVectorsInDataset() async throws {
        ...
        for result in results {
            let idx = Int(result.id.stableID)
            if idx >= duplicateStartIdx && idx < duplicateStartIdx + numDuplicates {
                duplicatesFound += 1
                duplicateIndices.append(result.id.stableID)
            }
        }
    ...
    func testZeroVector() async throws {
        ...
        print("  First result: index=\(firstResult.id.stableID), distance=\(firstResult.distance)")

        XCTAssertEqual(
            Int(firstResult.id.stableID), zeroIdx,
    ...
    func testHighDimensionalVectors() async throws {
        ...
        for query in queries {
            let gtResults = try await flatIndex.search(query: query, k: k)
            let gtIndices = Set(gtResults.map { $0.id.stableID })
            let gtDistances = Dictionary(uniqueKeysWithValues: gtResults.map { ($0.id.stableID, $0.distance) })
            ...
            for result in ivfResults {
                if let gtDist = gtDistances[result.id.stableID] {
    ...
    func testVariousDatasetSizes() async throws {
        ...
        for (sizeIdx, size) in sizes.enumerated() {
            ...
            for query in queries {
                let gtResults = try await flatIndex.search(query: query, k: k)
                groundTruth.append(Set(gtResults.map { $0.id.stableID }))
            }
    ...
    func testBatchAPIMatchesSingleQueryResults() async throws {
        ...
        var singleQueryResults: [SearchResults<VectorHandle>] = []
        ...
        for (queryIdx, (singleRes, batchRes)) in zip(singleQueryResults, batchResults).enumerated() {
            ...
            let singleIndices = singleRes.map { $0.id.stableID }
            let batchIndices = batchRes.map { $0.id.stableID }
    ...
    func testBatchAPIWithFilterFallsBackToSequential() async throws {
        ...
        for (queryIdx, (noFilterRes, withFilterRes)) in zip(noFilterResults, withFilterResults).enumerated() {
            ...
            let noFilterIndices = noFilterRes.map { $0.id.stableID }
            let withFilterIndices = withFilterRes.map { $0.id.stableID }
            XCTAssertEqual(
    ...
    func testBatchAPIRoutingThreshold() async throws {
        ...
        for (queryIdx, (ivfRes, flatRes)) in zip(ivfResults, flatResults).enumerated() {
            let ivfIndices = Set(ivfRes.map { $0.id.stableID })
            let flatIndices = Set(flatRes.map { $0.id.stableID })
