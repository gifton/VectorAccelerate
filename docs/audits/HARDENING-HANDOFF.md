...
**Slice 23 completed: neural `normalizeLatent` follow-up fixed.** The three exported
specialized quantizers normalize after bias/ReLU, and tiled pass 2 honors the dispatch
flag using the generic norm cutoff and normalized scale floor. Disabled behavior stays
unchanged. Raw pass 2 requires UInt32 buffer(5); see
[the normalization contract](../stability/NEURAL-LATENT-NORMALIZATION-CONTRACT.md).
...
and its two linked implementation plans separate the decisions. The owner subsequently
authorized the recommended 64 MiB pool cap with recoverable errors for slice 28. The owner approved optional-bias implementation and
the existing raw batch method becoming throwing; slice 27 records that work. The owner
separately selected the factory upload issue, addressed in slice 26 below. VA3-024 remains
subsequent performance work; the remaining open findings are not closed by these plans.
...
At the slice 26 checkpoint, global pool policy and optional-bias API decisions remained
pending; decision B was subsequently approved for slice 27. No numbered shader
finding is newly closed by this adjacent fix. Same checkpoint branch.

**Slice 27 completed: optional-bias validation fixed.** The owner approved the existing
raw batch `encodeFused` becoming throwing; external callers add `try`. Active bias gets
checked byte counts, minimum capacity and device validation before encoder mutation;
disabled bias uses a persistent one-Float binding. Empty batch grids return without
invalid zero-sized dispatches. Requested high-level bias allocation failure throws.
Float-only neural encoding reuses its persistent zero bias and guards raw latent widths
before binding. Ten new tests; targeted 55/0, API+shader validation debug/release 10/0,
full debug/release 1732/0/11 (1721 passed, 11 existing skips), all exit 0. Review approved
and source hashes matched. Allocation failure and wrong-device branches are source-reviewed
on this host; see the [optional-bias contract](../stability/OPTIONAL-BIAS-CONTRACT.md).
The raw batch residual and float-only binding gap are closed. Pool policy was still pending
at this checkpoint and was subsequently resolved for slice 28. Same single checkpoint
branch, no shader ABI or public dimension-cap changes.

**Slice 28 completed: central pool allocation bounds fixed.** The owner authorized the
recommended 64 MiB cap with recoverable errors. Original sizes are validated before capped
bucket lookup; typed counts and alignment rounding use checked arithmetic. Successful leases
have sufficient actual storage; zero/empty uploads preserve their existing semantics.
Preallocation rejects invalid counts and overflowing/over-budget groups without traps.
The bucketed factory returns nil for unsupported requests; direct allocations remain available.
IVF/neural local capacity guards remain, and IVF tests pin exact early-cap error details.
Eleven new tests; targeted pool31/0, IVF8/0, neural6/0; final API+shader validation25/0 in
both configurations. Full debug/release1743/0/11 (1732 passed, 11 existing skips), all exit0.
Final hashes matched and independent review approved. See the
[pool contract](../stability/BUFFER-ALLOCATION-CONTRACT.md). A separate pending-return
actor-address reuse/accounting concern was recorded at this checkpoint and subsequently
fixed in slice 29; slice 28 itself did not redesign pool lifecycle. Same single checkpoint branch; no new numbered shader finding is closed.

**Slice 29 completed: BufferPool pending-return lifecycle fixed.** Per-pool queues replace
the global actor-address registry; tokens own their buffer and weakly reference their
original queue. Pool destruction releases undrained returns and no longer cycles through
compatibility tokens. Reset retires the queue, so late explicit/deinit returns cannot enter
fresh accounting; compatibility handles remain releasable while the pool lives. Untracked
returns are rejected. Previous fresh-pool reset workarounds are removed. Nine new tests;
expanded red 31 assertions, final target 72/0, API+shader validation 9/0 in each configuration;
full debug/release 1752/0/11 (1741 passed, 11 existing skips), all exit 0. Review approved and
source hashes matched. See the [lifecycle contract](../stability/BUFFER-POOL-LIFECYCLE-CONTRACT.md)
for reset-generation budgets, provider lifetime for borrowed pointers, and GPU requirements.
ArgumentTablePool's analogous registry was subsequently fixed in slice 30; BufferPool
clearCache accounting remained a separate follow-up at that checkpoint.
Same checkpoint branch; no public signatures, shaders or allocation limits changed.

**Slice 30 completed: ArgumentTablePool ownership fixed.** Tokens weakly reference a
pool-owned queue; orphaned tables/bindings are released with the pool, while live tokens
own tables independently. Twelve direct engine returns migrated to token acquisition/defer
release, preserving descriptors and GPU lifetimes. Six new tests; baseline six assertions,
targeted 69/0, scoped table/engine API+shader validation 26/0 in debug and release; full
1758/0/11 each (1747 passed), all exit 0. Review approved and hashes matched. Broader
...
**Slice 33 completed: four more IVF placeholders implemented.** Tests cover automatic
training and refreshed exact recall after later inserts on a controlled separated fixture,
serialized nearest-centroid assignments/ties, common dimensions and both initializers'
metric contract. Exact results are checked through 768 dimensions; 1024/1536 reject under
the existing fused coarse-path cap. Euclidean remains the only supported metric. No
production/API/shader changes. Four fault injections were detected and restored; review
approved. All nine revived tests pass API-only validation 9/0 in debug/release; full gates
1764/0/2 each (1762 passed), all exit 0, hashes matched. Only the two throughput placeholders
...
| 27 | 09-13 | Optional bias validation | Throwing raw batch bounds, disabled bindings, empty-grid no-op, requested allocation failure and neural float fallback | 1732/0/11 |
...
| `OptionalBiasValidationTests` | Float neural no-bias binding, signed projections/activation, ragged/cap shapes, canaries, reload, raw explicit-bias ordering |
| `BatchOptionalBiasValidationTests` | Active bias layouts/capacity/overflow, disabled modes, legacy flags, encoder recovery, zero work and high-level results; conditional device coverage |
...
Slice 33 implements four more: recall through automatic training and later insertions,
nearest-centroid assignments with exact ties, common dimensions, and the Euclidean-only
metric contract through both initializers. Dimensions through 768 check exact results;
1024/1536 check the retained fused coarse-path rejection, without adding dimension support.
Only two placeholders remain: nprobe throughput scaling and throughput stability. They
are unimplemented performance tests, not environment gates. Their old strict linear-scaling
and low-variance assumptions need a controlled benchmark design.
...
**Coverage gaps / debt:** central pool allocation bounds are addressed in slice 28;
...
physically live retired leases by the documented policy. Direct factory
